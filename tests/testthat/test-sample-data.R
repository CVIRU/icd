...
test_that("uranium data looks okay", {
  ...
  expect_true(is.icd_decimal_diag(uranium_pathology$icd10))
...
test_that("vermont data looks okay", {
  ...
  expect_true(
    all(
      icd_is_valid.icd9(
        icd_wide_to_long(vermont_dx)$icd_code)))

  expect_true(
    all(
      icd_wide_to_long(vermont_dx)$icd_code %in% icd::icd9cm_hierarchy$code))
